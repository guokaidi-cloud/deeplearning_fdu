...
Model::Model(string onnx_path, logger::Level level, Params params) {
  ...
  m_enginePath =
      changePath(onnx_path, "../engine", ".engine", getPrec(params.prec));
}
...
void Model::init_model() {
  /* 一个model的engine,
   * context这些一旦创建好了，当多次调用这个模型的时候就没必要每次都初始化了*/
...
bool Model::build_engine() {
  ...
  if (builder->platformHasFastFp16() && m_params->prec == model::FP16) {
    ...
    // 只在 INT8 模式下才创建校准器
    calibrator.reset(new Int8EntropyCalibrator(
        64, "calibration/calibration_list.txt",
        "calibration/calibration_table.txt", 3 * 640 * 640, 640, 640));
    config->setInt8Calibrator(calibrator.get());
  }

  auto engine =
      shared_ptr<ICudaEngine>(builder->buildEngineWithConfig(*network, *config),
                              destroy_trt_ptr<ICudaEngine>);
  auto plan = builder->buildSerializedNetwork(*network, *config);
  auto runtime = shared_ptr<IRuntime>(createInferRuntime(*m_logger),
...
void Model::print_network(INetworkDefinition &network, bool optimized) {
  ...
  for (int i = 0; i < inputCount; i++) {
    ...
    LOGV("Input info: %s:%s", input->getName(),
         printTensorShape(input).c_str());
  }

  for (int i = 0; i < outputCount; i++) {
    auto output = network.getOutput(i);
    LOGV("Output info: %s:%s", output->getName(),
  ...
  if (!optimized) {
    for (int i = 0; i < layerCount; i++) {
      ...
      LOGV("layer_info: %-40s:%-25s->%-25s[%s]", layer->getName(),
           printTensorShape(input).c_str(), printTensorShape(output).c_str(),
           getPrecision(layer->getPrecision()).c_str());
    }

  } else {
    auto inspector =
        shared_ptr<IEngineInspector>(m_engine->createEngineInspector());
    for (int i = 0; i < layerCount; i++) {
      LOGV("layer_info: %s", inspector->getLayerInformation(
